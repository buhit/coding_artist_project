...
display(): 타겟을 화면에 정웅재 캐릭터 디자인으로 그린다. 타겟의 중앙에 남은 수명의 숫자를 표시해준다.
...
class Target {
  ...
  float vx, vy;
  float radius;
  int hitCount; // 맞춰야 하는 총알의 개수

  Target(float x, float y, float radius, int hitCount ) {
    this.x = x;
    this.y = y;
    this.radius = radius;
    this.hitCount = hitCount; // 초기화
    
    float speedFactor = 10 / sqrt(hitCount); 
